...
import {
  ArrowRight, Bookmark, Building2, Camera, ChevronRight, Clock3, FileText, Image as ImageIcon,
  MapPin, Megaphone, Search, ShieldCheck, Star, UserRound, Zap,
} from 'lucide-react'
  ...
  return (
    <span className="inline-flex items-center gap-[3px]" aria-label={`${rating} out of 5`}>
      {[0, 1, 2, 3, 4].map((i) => (
        <Star
          key={i}
          className={`${className} ${i < rounded ? 'fill-[var(--slot4-accent)] text-[var(--slot4-accent)]' : 'fill-[#38393a] text-[#38393a]'}`}
        />
      ))}
    </span>
  )
...
const valueTiles = [
  ...
  { icon: Bookmark, label: 'Curated bookmarks' },
  { icon: MapPin, label: 'Local discovery' },
  { icon: Zap, label: 'Free to submit' },
  { icon: Clock3, label: 'Updated daily' },
...
export function EditableHomeHero({ primaryTask, primaryRoute, posts, timeSections }: HomeSectionProps) {
  ...
  const heroTitle = pagesContent.home.hero.title || [`Discover local businesses,`, `save what matters online.`]
  ...
  return (
    <section className="relative">
      <div className="relative h-[440px] w-full overflow-hidden sm:h-[520px] lg:h-[560px]">
        ...
        <div className={`relative flex h-full flex-col justify-center ${container}`}>
          <div className="max-w-2xl">
            ...
            <h1 className="editable-display mt-3 text-balance text-4xl font-medium leading-[1.1] tracking-[-0.02em] text-white sm:text-5xl lg:text-[3.75rem]">
              {heroTitle.map((line, index) => (
                <span key={line} className="block">
                  {index === heroTitle.length - 1 ? <span className="text-white/70">{line}</span> : line}
                </span>
              ))}
            </h1>
            <p className="mt-4 max-w-xl text-base text-white/90 sm:text-lg">{pagesContent.home.hero.description}</p>

            <form action="/search" className="mt-7 flex w-full max-w-xl overflow-hidden rounded-[var(--editable-radius-pill)] bg-white shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
              <div className="flex flex-1 items-center gap-2.5 px-5">
                <Search className="h-5 w-5 shrink-0 text-[var(--slot4-muted-text)]" />
                <input
                  name="q"
                  placeholder="Search listings, bookmarks, categories…"
                  className="w-full bg-transparent py-4 text-sm text-[var(--slot4-page-text)] outline-none placeholder:text-[var(--slot4-muted-text)]"
                />
              </div>
              <button className="shrink-0 bg-[var(--slot4-accent)] px-6 text-sm font-semibold text-white transition hover:brightness-95 sm:px-8">
                Search
              </button>
            </form>
            ...
            <div className="mt-6 flex flex-wrap gap-2.5">
              {categories.map((task) => (
                <Link
                  key={task.key}
                  href={task.route}
                  className="rounded-[var(--editable-radius-pill)] border border-white/30 bg-white/10 px-4 py-1.5 text-sm font-medium text-white backdrop-blur-sm transition hover:bg-white/20"
                >
                  {task.label}
                </Link>
              ))}
      ...
      {/* Value-prop tile row under the hero */}
      <div className="border-b border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]">
        <div className={`grid grid-cols-2 gap-3 py-6 sm:grid-cols-3 lg:grid-cols-6 ${container}`}>
          {valueTiles.map((tile) => (
            <div key={tile.label} className="flex flex-col items-center gap-2 text-center sm:flex-row sm:text-left">
              ...
              <span className="text-xs font-medium leading-tight text-[var(--slot4-muted-text)] sm:text-sm">{tile.label}</span>
            </div>
          ))}
        </div>
        <div className={`flex justify-center border-t border-[var(--editable-border)] py-3 ${container}`}>
          <Link href={primaryRoute} className="inline-flex items-center gap-1 text-sm font-semibold text-[var(--slot4-accent)] hover:underline">
    ...
    <section className="border-t border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]">
      <div className={`py-12 sm:py-14 ${container}`}>
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="editable-display text-2xl font-normal tracking-[-0.01em] sm:text-3xl">Browse by category</h2>
...
function ActivityCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-[var(--editable-radius-card)] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] transition duration-300 hover:-translate-y-1.5 hover:border-[var(--slot4-accent)]/30">
      ...
      <div className="flex flex-1 flex-col px-4 py-4">
        <Link href={href} className="text-lg font-semibold leading-snug tracking-[-0.01em] text-[var(--slot4-page-text)] hover:text-[var(--slot4-accent)]">
          {post.title}
        </Link>
        <RatingRow post={post} />
    ...
    <section className="border-t border-[var(--editable-border)] bg-[var(--slot4-warm)]">
      <div className={`py-14 sm:py-16 ${container}`}>
        <div className="text-center">
          <h2 className="editable-display text-3xl font-normal tracking-[-0.01em] sm:text-4xl">Recent activity</h2>
          <p className="mx-auto mt-3 max-w-2xl text-[var(--slot4-muted-text)]">
            The latest listings and bookmarks shared across {SITE_CONFIG.name}.
          </p>
        </div>
        ...
        <div className="mt-10 text-center">
          <Link href={primaryRoute} className="inline-flex items-center gap-2 rounded-[var(--editable-radius-pill)] border border-[var(--editable-border)] px-6 py-3 text-sm font-semibold text-[var(--slot4-page-text)] transition hover:border-[var(--slot4-accent)] hover:text-[var(--slot4-accent)]">
...
function CompactCard({ post, href }: { post: SitePost; href: string }) {
  ...
  return (
    <Link
      href={href}
      className="group flex flex-col overflow-hidden rounded-[var(--editable-radius-card)] border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] transition duration-300 hover:-translate-y-1.5 hover:border-[var(--slot4-accent)]/30"
    >
      <div className="relative aspect-[3/2] overflow-hidden bg-[var(--slot4-media-bg)]">
        <img src={image} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" />
        {category ? (
          <span className="absolute left-3 top-3 rounded-full bg-black/60 px-3 py-1 text-[11px] font-semibold text-white backdrop-blur-sm">{category}</span>
        ) : null}
      </div>
  ...
  return (
    <>
      {visible.map((section, index) => {
        ...
        return (
          <section key={section.key} className={index % 2 === 0 ? 'border-t border-[var(--editable-border)] bg-[var(--slot4-surface-bg)]' : 'border-t border-[var(--editable-border)] bg-[var(--slot4-warm)]'}>
            <div className={`py-12 sm:py-14 ${container}`}>
              <div className="flex items-end justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--slot4-accent)]">{copy.eyebrow}</p>
                  <h2 className="editable-display mt-2 text-2xl font-normal tracking-[-0.01em] sm:text-3xl">{copy.title}</h2>
                </div>
...
export function EditableHomeCta() {
  return (
    <section id="get-app" className="scroll-mt-24 border-t border-[var(--editable-border)] bg-[#0a0a0a]">
      <div className={`flex flex-col items-center gap-6 py-16 text-center sm:py-20 ${container}`}>
        <h2 className="editable-display max-w-2xl text-3xl font-normal tracking-[-0.02em] text-white sm:text-4xl">
          Got something worth sharing?
        </h2>
        <p className="max-w-xl text-base text-white/60 sm:text-lg">
          Add your business, save a bookmark, or share a resource — and reach the {SITE_CONFIG.name} community.
        </p>
        <div className="flex flex-wrap justify-center gap-4">
          <Link href="/create" className="inline-flex items-center gap-2 rounded-[var(--editable-radius-pill)] bg-[var(--slot4-accent)] px-7 py-3 text-sm font-semibold text-white transition hover:brightness-110">
            Submit a post
          </Link>
          <Link href="/contact" className="inline-flex items-center gap-2 rounded-[var(--editable-radius-pill)] border border-white/20 px-7 py-3 text-sm font-semibold text-white transition hover:bg-white/10">
            Contact us
          </Link>
        </div>
      </div>
