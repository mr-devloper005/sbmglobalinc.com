  ...
  return (
    <>
      <article className="mx-auto max-w-4xl px-6 py-14 sm:py-20">
        ...
        <h1 className="editable-display mt-5 text-balance text-4xl font-normal leading-[1.06] tracking-[-0.03em] sm:text-5xl lg:text-[3.4rem]">{post.title}</h1>
...
function ListingDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <>
      {/* Hero header band with gradient accent */}
      <div className="relative overflow-hidden border-b border-[var(--tk-line)]">
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-[var(--tk-accent)]/[0.03] via-transparent to-transparent" />
        <div className="pointer-events-none absolute -left-40 -top-40 h-[400px] w-[400px] rounded-full bg-[var(--tk-accent)]/[0.04] blur-[100px]" />
        <div className="mx-auto max-w-[var(--editable-container)] px-6 pb-14 pt-10 sm:pb-16 sm:pt-14 lg:px-8">
          <BackLink task="listing" />

          <div className="mt-10 flex flex-col gap-6 sm:mt-12 sm:flex-row sm:items-start sm:gap-8">
            {/* Logo tile */}
            <div className="relative flex h-28 w-28 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-[var(--tk-line)] bg-[var(--tk-surface)]">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h1 className="editable-display mt-3 max-w-3xl text-balance text-4xl font-normal leading-[1.04] tracking-[-0.03em] sm:text-5xl lg:text-[3.25rem]">{post.title}</h1>
              <DetailMeta post={post} category={category} />
            </div>
          </div>

          {leadText(post) ? <p className="mt-7 max-w-2xl text-lg leading-8 text-[#c7c7c7]">{leadText(post)}</p> : null}

          {/* Quick action pills in header */}
          <div className="mt-8 flex flex-wrap items-center gap-3">
            {website ? (
              <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-[var(--editable-radius-control)] border border-[var(--tk-accent)] bg-[var(--tk-accent)] px-5 py-2.5 text-sm font-semibold text-white transition hover:brightness-110">
                Visit website <ArrowUpRight className="h-4 w-4" />
              </Link>
            ) : null}
            {phone ? (
              <a href={`tel:${phone}`} className="inline-flex items-center gap-2 rounded-[var(--editable-radius-control)] border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 text-sm font-medium text-[var(--tk-text)] transition hover:border-[var(--tk-accent)]">
                <Phone className="h-4 w-4 text-[var(--tk-accent)]" /> {phone}
              </a>
            ) : null}
            {email ? (
              <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-[var(--editable-radius-control)] border border-[var(--tk-line)] bg-[var(--tk-surface)] px-5 py-2.5 text-sm font-medium text-[var(--tk-text)] transition hover:border-[var(--tk-accent)]">
                <Mail className="h-4 w-4 text-[var(--tk-accent)]" /> Email
              </a>
            ) : null}
          </div>
        </div>
        ...
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_380px]">
          <article className="min-w-0">
            ...
            {gallery.length > 0 ? (
              <section className="mt-10">
                <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--tk-muted)]">Gallery</p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {gallery.slice(0, 6).map((image, index) => (
                    <div key={`${image}-${index}`} className="group relative aspect-[4/3] overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-raised)]">
                      <img src={image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" />
                      <div className="pointer-events-none absolute inset-0 rounded-[var(--tk-radius)] ring-1 ring-inset ring-white/[0.06]" />
                    </div>
                  ))}
                </div>
              </section>
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6">
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--tk-muted)]">Business details</p>
              <div className="mt-5 space-y-4">
                {address ? (
                  <div className="flex items-start gap-3 text-sm">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><MapPin className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div><p className="font-medium text-[var(--tk-text)]">{address}</p><p className="text-xs text-[var(--tk-muted)]">Location</p></div>
                  </div>
                ) : null}
                {phone ? (
                  <div className="flex items-center gap-3 text-sm">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Phone className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div><p className="font-medium text-[var(--tk-text)]">{phone}</p><p className="text-xs text-[var(--tk-muted)]">Phone</p></div>
                  </div>
                ) : null}
                {email ? (
                  <div className="flex items-center gap-3 text-sm">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Mail className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div className="min-w-0"><p className="truncate font-medium text-[var(--tk-text)]">{email}</p><p className="text-xs text-[var(--tk-muted)]">Email</p></div>
                  </div>
                ) : null}
                {website ? (
                  <div className="flex items-center gap-3 text-sm">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Globe2 className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div className="min-w-0"><p className="truncate font-medium text-[var(--tk-text)]">{cleanDomain(website)}</p><p className="text-xs text-[var(--tk-muted)]">Website</p></div>
                  </div>
                ) : null}
                {category ? (
                  <div className="flex items-center gap-3 text-sm">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Tag className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div><p className="font-medium text-[var(--tk-text)]">{category}</p><p className="text-xs text-[var(--tk-muted)]">Category</p></div>
                  </div>
                ) : null}
              </div>
              {/* CTA inside card */}
              {website ? (
                <Link href={website} target="_blank" rel="noreferrer" className="mt-6 flex w-full items-center justify-center gap-2 rounded-[var(--editable-radius-control)] bg-[var(--tk-accent)] px-5 py-3 text-sm font-semibold text-white transition hover:brightness-110">
                  Visit website <ExternalLink className="h-4 w-4" />
                </Link>
              ) : null}
            </div>

            <Ads slot="sidebar" size={pickAdVariant('sidebar')} showLabel className="w-full" />
      ...
      <section className="mx-auto grid max-w-[var(--editable-container)] gap-10 px-6 py-14 sm:py-20 lg:grid-cols-[360px_minmax(0,1fr)] lg:px-8">
        <aside className="lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="mt-7 rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-7">
            ...
            <h1 className="editable-display mt-4 text-2xl font-normal leading-tight tracking-[-0.02em]">{post.title}</h1>
            <DetailMeta post={post} category={getField(post, ['category'])} />
            <p className="editable-display mt-6 text-4xl font-normal tracking-[-0.03em] text-[var(--tk-accent)]">{price || 'Open offer'}</p>
      ...
      <section className="mx-auto max-w-[var(--editable-container)] px-6 py-14 sm:py-20 lg:px-8">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1.4fr_0.6fr]">
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            ...
            <h1 className="editable-display mt-6 text-4xl font-normal leading-[1.05] tracking-[-0.03em] sm:text-5xl">{post.title}</h1>
...
// ----- Bookmark: a single curated resource (no hero image — metadata-first) -----
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  const website = getField(post, ['website', 'url', 'link'])
  const category = categoryOf(post, '')
  const tags = (post.tags || []).filter((tag) => tag && tag !== category).slice(0, 5)
  const rating = ratingOf(post)
  const filled = Math.round(rating)
  return (
    <>
      {/* Hero header band — no image, rich typography */}
      <div className="relative overflow-hidden border-b border-[var(--tk-line)]">
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[var(--tk-accent)]/[0.04] via-transparent to-transparent" />
        <div className="pointer-events-none absolute -right-40 -top-40 h-[500px] w-[500px] rounded-full bg-[var(--tk-accent)]/[0.03] blur-[120px]" />
        <div className="mx-auto max-w-[var(--editable-container)] px-6 pb-14 pt-10 sm:pb-20 sm:pt-14 lg:px-8">
          <BackLink task="sbm" />

          <div className="mt-12 flex items-start gap-6 sm:mt-16">
            <div className="relative flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl border border-[var(--tk-accent)]/20 bg-gradient-to-br from-[var(--tk-accent)]/[0.15] to-[var(--tk-accent)]/[0.05]">
              <Bookmark className="h-9 w-9 text-[var(--tk-accent)]" />
              <div className="absolute -inset-px rounded-2xl ring-1 ring-inset ring-white/[0.06]" />
            </div>
            <div className="min-w-0 flex-1">
              <Kicker task="sbm">Saved resource</Kicker>
              <h1 className="editable-display mt-3 max-w-3xl text-balance text-4xl font-normal leading-[1.05] tracking-[-0.03em] sm:text-5xl lg:text-[3.25rem]">{post.title}</h1>
            </div>
          </div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm font-medium text-[var(--tk-text)]">{rating.toFixed(1)}</span>
            <span className="text-sm text-[var(--tk-muted)]">{reviewsOf(post)} reviews</span>
            {category ? (
              <>
                <span className="h-1 w-1 rounded-full bg-[var(--tk-muted)] opacity-40" />
                <span className="rounded-full bg-[var(--tk-accent-soft)] px-3 py-1 text-xs font-semibold text-[var(--tk-accent)]">{category}</span>
              </>
            ) : null}
            {tags.map((tag) => (
              <span key={tag} className="inline-flex items-center gap-1 rounded-full border border-[var(--tk-line)] px-3 py-1 text-xs font-medium text-[var(--tk-muted)]">
                <Tag className="h-3 w-3" /> {tag}
              </span>
            ))}
          </div>

          {leadText(post) ? <p className="mt-6 max-w-2xl text-lg leading-8 text-[#c7c7c7]">{leadText(post)}</p> : null}

          {/* CTA row */}
          <div className="mt-8 flex flex-wrap items-center gap-4">
            {website ? (
              <Link href={website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2.5 rounded-[var(--editable-radius-control)] border border-[var(--tk-accent)] bg-[var(--tk-accent)] px-6 py-3 text-sm font-semibold text-white transition hover:brightness-110">
                Visit resource <ArrowUpRight className="h-4 w-4" />
              </Link>
            ) : null}
            {website ? (
              <span className="inline-flex items-center gap-2 rounded-[var(--editable-radius-control)] border border-[var(--tk-line)] bg-[var(--tk-surface)] px-4 py-2.5 text-sm text-[var(--tk-muted)]">
                <Globe2 className="h-4 w-4 text-[var(--tk-accent)]" /> {cleanDomain(website)}
              </span>
            ) : null}
          </div>
        </div>
      ...
      <div className="mx-auto max-w-[var(--editable-container)] px-6 py-14 sm:py-20 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_340px]">
          <article className="min-w-0">
            <BodyContent post={post} />
            <div className="mt-14">
             <Ads slot="article-bottom" size={pickAdVariant('article-bottom')} showLabel className="w-full" />
            </div>
          </article>
          ...
          <aside className="space-y-6 lg:sticky lg:top-24 lg:self-start">
            {/* About card */}
            <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6">
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--tk-muted)]">About this bookmark</p>
              <div className="mt-5 space-y-4">
                <div className="flex items-center gap-3 text-sm">
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Bookmark className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                  <div><p className="font-medium text-[var(--tk-text)]">Curated resource</p><p className="text-xs text-[var(--tk-muted)]">Verified by {SITE_CONFIG.name}</p></div>
                </div>
                {category ? (
                  <div className="flex items-center gap-3 text-sm">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Tag className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div><p className="font-medium text-[var(--tk-text)]">{category}</p><p className="text-xs text-[var(--tk-muted)]">Category</p></div>
                  </div>
                ) : null}
                {website ? (
                  <div className="flex items-center gap-3 text-sm">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><Globe2 className="h-4 w-4 text-[var(--tk-accent)]" /></div>
                    <div className="min-w-0"><p className="truncate font-medium text-[var(--tk-text)]">{cleanDomain(website)}</p><p className="text-xs text-[var(--tk-muted)]">Source</p></div>
                  </div>
                ) : null}
              </div>
            </div>
            ...
            {related.length ? (
              <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6">
                <div className="flex items-center justify-between gap-3">
                  <h2 className="editable-display text-lg font-normal tracking-[-0.02em]">Related bookmarks</h2>
                  <Link href={getTaskConfig('sbm')?.route || '/sbm'} className="text-xs font-medium uppercase tracking-[0.14em] text-[var(--tk-accent)]">View all</Link>
                </div>
        ...
        <article className="min-w-0">
          <div className="flex items-center gap-5">
            ...
            <div className="min-w-0">
              ...
              <h1 className="editable-display mt-3 text-3xl font-normal leading-[1.05] tracking-[-0.02em] sm:text-4xl">{post.title}</h1>
            </div>
          </div>
          <BodyContent post={post} />
          {fileUrl ? (
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-8 text-center">
              ...
              <h1 className="editable-display mt-6 text-2xl font-normal tracking-[-0.02em]">{post.title}</h1>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {visible.map(([label, value, Icon]) => (
        <div key={label} className="group rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-5 transition hover:border-[var(--tk-accent)]/30">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--tk-accent-soft)]">
              <Icon className="h-4.5 w-4.5 text-[var(--tk-accent)]" />
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--tk-muted)]">{label}</p>
              <p className="mt-0.5 break-words text-sm font-medium leading-6 text-[var(--tk-text)]">{value}</p>
            </div>
          </div>
        </div>
      ))}
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)]">
      <div className="flex items-center gap-3 border-b border-[var(--tk-line)] p-4">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--tk-accent-soft)]"><MapPin className="h-4 w-4 text-[var(--tk-accent)]" /></div>
        <div className="min-w-0">
          <p className="truncate text-sm font-medium">{label || 'Map location'}</p>
          <p className="text-[11px] text-[var(--tk-muted)]">Location</p>
        </div>
      </div>
...
function BadgeLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-[var(--editable-radius-control)] border border-[var(--tk-line)] bg-[var(--tk-raised)] px-4 py-3 text-sm">
  ...
  return (
    <div className="space-y-6">
      ...
      {related.length ? (
        <div className="rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] p-6">
          <div className="flex items-center justify-between gap-3">
            <h2 className="editable-display text-lg font-normal tracking-[-0.02em]">More like this</h2>
...
function RelatedStrip({ task, related }: { task: TaskKey; related: SitePost[] }) {
  ...
  const taskConfig = getTaskConfig(task)
  return (
    <section className="border-t border-[var(--tk-line)] bg-[var(--tk-surface)]/50">
      <div className="mx-auto max-w-[var(--editable-container)] px-6 py-16 sm:py-20 lg:px-8">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.2em] text-[var(--tk-accent)]">Explore more</p>
            <h2 className="editable-display mt-2 text-2xl font-normal tracking-[-0.02em] sm:text-3xl">More {(taskConfig?.label || 'posts').toLowerCase()}</h2>
          </div>
          <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-1.5 rounded-[var(--editable-radius-control)] border border-[var(--tk-line)] px-4 py-2 text-sm font-medium text-[var(--tk-muted)] transition hover:border-[var(--tk-accent)] hover:text-[var(--tk-text)]">View all <ArrowUpRight className="h-4 w-4" /></Link>
        </div>
...
function RelatedCard({ task, post, grid = false }: { task: TaskKey; post: SitePost; grid?: boolean }) {
  ...
  if (grid) {
    return (
      <Link href={href} className="group block overflow-hidden rounded-[var(--tk-radius)] border border-[var(--tk-line)] bg-[var(--tk-surface)] transition duration-300 hover:-translate-y-1 hover:border-[var(--tk-accent)]/30">
        ...
        <div className="p-5">
          <h3 className="editable-display line-clamp-2 text-base font-normal leading-snug tracking-[-0.01em] group-hover:text-[var(--tk-accent)]">{post.title}</h3>
          <p className="mt-2.5 line-clamp-2 text-sm leading-6 text-[var(--tk-muted)]">{stripHtml(summaryText(post))}</p>
          <span className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-[var(--tk-accent)] opacity-0 transition group-hover:opacity-100">Read more <ArrowUpRight className="h-3 w-3" /></span>
        </div>
      </Link>
    )
  }
  return (
    <Link href={href} className="group flex gap-4 rounded-xl border border-[var(--tk-line)] p-3.5 transition hover:border-[var(--tk-accent)]/40 hover:bg-[var(--tk-raised)]/50">
      {image && task !== 'sbm' ? (
        ...
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-[var(--tk-raised)]">
          {task === 'sbm' ? <Bookmark className="h-5 w-5 text-[var(--tk-accent)]" /> : <FileText className="h-5 w-5 text-[var(--tk-muted)]" />}
        </div>
      )}
      <div className="min-w-0">
        <h3 className="line-clamp-2 text-sm font-medium leading-snug tracking-[-0.01em] group-hover:text-[var(--tk-accent)]">{post.title}</h3>
